...
export async function POST(
  ...
  try {
    ...
    if (action === 'join') {
      ...
      const result = await GameStore.joinRoom(code, playerId, playerNickname, asSpectator);
      if (!result.success) {
    ...
    if (action === 'start') {
      const result = await GameStore.startGame(code);
      if (!result.success) {
        return NextResponse.json({ error: result.error }, { status: 400 });
      }
    ...
    if (action === 'leave') {
      await GameStore.leaveRoom(code, playerId);
    ...
    if (action === 'reset') {
      ...
      const result = await GameStore.resetRoom(code);
      if (!result.success) {
        return NextResponse.json({ error: result.error }, { status: 400 });
      }
      return NextResponse.json({ success: true });
...
export async function DELETE(
  ...
  const { code } = await params;
  await GameStore.deleteRoom(code);
  return NextResponse.json({ success: true });
}
